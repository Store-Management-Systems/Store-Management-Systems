...
const exportLedgerExcel = async (req, res) => {
    ...
    try {
        const person = await db.prepare(`SELECT * FROM people WHERE id = ? AND shop_id = ?`).get(personId, activeShop);
        if (!person)
            return error(res, 'Person not found', 404);
        let sql = `SELECT * FROM ledgers WHERE person_id = ? AND shop_id = ?`;
        const params = [personId, activeShop];
        if (from && to) {
            sql += ` AND created_at BETWEEN ? AND ?`;
...
const exportLedgerPdf = async (req, res) => {
    ...
    try {
        ...
        if (person.business_name)
            doc.fontSize(12).text(person.business_name, { align: 'center' });
        ...
        entries.forEach((e, idx) => {
            ...
            doc.fontSize(10).text(`${idx + 1}. [${new Date(e.created_at).toLocaleDateString()}] ${e.entry_type} | Dr: ₹${debit.toFixed(2)} | Cr: ₹${credit.toFixed(2)} | Bal: ₹${running.toFixed(2)}`);
        });
        doc.moveDown();

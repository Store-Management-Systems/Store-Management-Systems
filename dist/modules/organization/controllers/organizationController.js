...
const createOrganization = async (req, res) => {
    ...
    const { name, code, owner_name, owner_username, owner_password, email, phone, subscription_plan = 'Standard', subscription_expiry, price_per_branch = 999 } = req.body;
    if (!name || !code) {
...
const assignOwner = async (req, res) => {
    ...
    try {
        ...
        if (!targetOwnerId && owner_username && owner_password) {
            ...
            if (!user)
                return error(res, 'Selected user not found', 404);

...
const createBill = async (req, res) => {
    try {
        ...
        const shopId = req.user.active_shop_id || req.user.shop_id;
        const orgId = req.user.organization_id;
        if (!items || !Array.isArray(items) || items.length === 0) {
            return (0, shared_1.error)(res, 'At least one item is required to generate a bill', 400);
        }
        const shop = await shared_1.db.prepare('SELECT * FROM shops WHERE id = ?').get(shopId);
        ...
        if (pgPool) {
            try {
                const pgRes = await pgPool.query('SELECT * FROM fn_calculate_bill_totals($1, $2, $3, $4)', [subtotal, taxRate, parseFloat(discount) || 0, discount_type || 'rupees']);
        ...
        for (const item of items) {
            ...
            if (!shared_1.db.isPg && (item.item_id || item.id)) {
                await shared_1.db.prepare('UPDATE items SET stock = MAX(0, stock - ?) WHERE id = ?').run(parseInt(item.quantity) || 1, item.item_id || item.id);
            }
        }
        const createdBill = await shared_1.db.prepare('SELECT * FROM bills WHERE id = ?').get(billId);
        const createdItems = await shared_1.db.prepare('SELECT * FROM bill_items WHERE bill_id = ?').all(billId);
        createdBill.items = createdItems;
        return (0, shared_1.success)(res, 'Bill generated successfully', createdBill, 201);
    }
    catch (err) {
        return (0, shared_1.error)(res, err.message || 'Failed to create bill', 500);
    }
};
...
const getBills = async (req, res) => {
    try {
        const shopId = req.user.active_shop_id || req.user.shop_id;
        const bills = await shared_1.db.prepare('SELECT * FROM bills WHERE shop_id = ? ORDER BY created_at DESC LIMIT 100').all(shopId);
        for (const b of bills) {
            b.items = await shared_1.db.prepare('SELECT * FROM bill_items WHERE bill_id = ?').all(b.id);
        }
        return (0, shared_1.success)(res, 'Bills retrieved successfully', bills);
    }
    catch (err) {
        return (0, shared_1.error)(res, err.message || 'Failed to retrieve bills', 500);
    }
};

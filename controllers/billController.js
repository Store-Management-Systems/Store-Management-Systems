...
const createBill = async (req, res) => {
    ...
    // Strict Validation 1: At least 1 product added
    if (!items || items.length === 0) {
        return error(res, 'At least one item is required to generate a bill', 400);
    }

    // Strict Validation 2: Customer Name non-empty
    const trimmedName = (customerName || '').trim();
    if (!trimmedName) {
        return error(res, 'Customer or B2B Party name is required', 400);
    }

    // Strict Validation 3: Phone number check (if provided, must be 10 numeric digits)
    ...
    try {
        ...
        `).run(
            ...
            calcSubtotal, calcTax, calcDiscount, calcTotal, paid, due, paymentStatus, effectivePaymentMode, splitJson, notes || null
        );
...
// Record Partial or Full Due Clearance Payment for an Existing Bill

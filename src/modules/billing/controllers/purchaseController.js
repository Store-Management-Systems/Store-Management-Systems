...
const getPurchaseById = async (req, res) => {
    ...
    try {
        ...
        if (!purchase) return error(res, 'Purchase record not found', 404);
...
const createPurchase = async (req, res) => {
    ...
    try {
        ...
        await db.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, ?, ?, ?, ?, ?, ?)
        `).run(
            purchaseId, activeShop, supplierId, req.user.id, purchaseNumber, supplier_invoice_no || null,
            subtotal, total, paid, due, paymentStatus, paymentMode, notes || 'B2B Restock Purchase'
        );
